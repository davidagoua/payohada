  ...
  <div v-else-if="contrat && salarie" class="max-w-3xl mx-auto p-8 bg-white min-h-screen flex flex-col justify-between text-slate-900 relative">
    ...
    <div class="space-y-8">
      ...
      <div class="space-y-6 text-sm leading-relaxed text-slate-800 pt-6">
        <p>
          La société <strong>{{ dossier.nom }}</strong>, représentée par son établissement <strong>{{ etab.nom }}</strong>,
        ...
        <p class="text-justify">
          a été employé(e) au sein de notre entreprise du <strong>{{ contrat.date_debut_contrat }}</strong> au 
        ...
        <p class="text-justify">
          Pendant toute cette période, Monsieur / Madame {{ salarie.prenom }} {{ salarie.nom }} a exercé ses fonctions avec loyauté, 
          conscience professionnelle et à notre entière satisfaction.
        </p>

        <p class="text-justify">
          Il/Elle quitte notre entreprise ce jour, libre de tout engagement envers elle.
        </p>
    ...
    <div class="flex justify-end pt-24">
      <div class="w-1/2 text-right space-y-16">
        <div class="text-sm">
          <p class="font-bold text-slate-800">Pour la Direction</p>
          <p class="text-xs text-slate-500 italic">Signature et cachet autorisés</p>
        </div>
      </div>
    </div>
